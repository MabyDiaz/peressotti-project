...
//          try {
//         const categoriaId = producto.idCategoria || producto.Categoria?.id

//         console.log("[v0] Loading related products for category:", categoriaId)

//         if (categoriaId) {
//           const response = await axios.get(`/api/productos/categoria/${categoriaId}`)
//           const productosCategoria = response.data.data || []

//           // Filter out current product
//           const relacionados = productosCategoria.filter((p) => p.id !== Number.parseInt(id))
//           setProductosRelacionados(relacionados.slice(0, 4))

//           console.log("[v0] Related products loaded:", relacionados.length)
//         }
//       } catch (error) {
...
const DetalleProducto = () => {
  ...
  const calcularPrecioConDescuento = (precio, descuento) =>
    Number.parseFloat((precio * (1 - descuento / 100)).toFixed(2));
  ...
  useEffect(() => {
    const fetchProducto = async () => {
      try {
        ...
        try {
          // Obtener el idCategoria del product data
          const categoriaId =
            productoRes.data.idCategoria || productoRes.data.Categoria?.id;

          if (categoriaId) {
            console.log('Loading related products for category:', categoriaId);
            ...
            console.log('Related products response:', relacionadosArray);

            // Filtrar el producto actual (4 productos)
            const relacionados = relacionadosArray
              .filter((p) => p.id !== Number.parseInt(id))
              .slice(0, 4);

            console.log('Filtered related products:', relacionados);
            setProductosRelacionados(relacionados);
          } else {
            console.warn('No category ID found for product:', productoRes.data);
  ...
  if (error || !producto) {
    return (
      <div className='container mx-auto px-4 py-8'>
        ...
        <button
          onClick={() => navigate(-1)}
          className='mt-4 bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded flex items-center transition-colors'>
  ...
  return (
    <div className='container mx-auto px-4 py-8'>
      <button
        onClick={() => navigate(-1)}
        className='mb-4 bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded flex items-center transition-colors'>
        <FaArrowLeft className='mr-2' />
        Volver
      </button>
